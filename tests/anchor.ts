...
describe("Test", () => {
  ...
  it("initialize_escrow", async () => {
   ...
   //await program.provider.connection.requestAirdrop(client.publicKey, 2 * web3.LAMPORTS_PER_SOL);
   ...
   const escrowAmount = 0.25 * web3.LAMPORTS_PER_SOL;
    ...
    const [escrowPda, escrowBump] = web3.PublicKey.findProgramAddressSync(
      [Buffer.from("escrow"), Buffer.from(bookingId), client.publicKey.toBuffer()],
      program.programId
    );

    console.log("Initializing Escrow...");

    const txHash = await program.methods.initializeEscrow(bookingId, verifyCode, new anchor.BN(escrowAmount))
    .accounts({
  ...
  it("release_escrow", async () => {
    ...
    const bookingId = "booking123"; // Test booking ID
    const verifyCode = "123456";   // Test verification code
    const escrowAmount = 0.5 * web3.LAMPORTS_PER_SOL;
    ...
    const txHash = await program.methods.releaseEscrow(verifyCode)
    .accounts({
      escrowAccount: escrowPda,
    ...
    const escortBalance = await program.provider.connection.getBalance(escort.publicKey);
